...
export function detectContainerEscape(
  ...
  if (PRIVILEGED_CONTAINER_PATTERNS.some((p) => p.test(toolInput))) {
    detections.push({
      ...
      message:
        'Privileged container detected - grants nearly all host capabilities, enables container escape',
    });
  }
  ...
  if (DOCKER_SOCKET_MOUNT_PATTERNS.some((p) => p.test(toolInput))) {
    detections.push({
      ...
      message:
        'Docker socket mount detected - gives container full control over host Docker daemon, equivalent to root access',
    });
  }
  ...
  if (HOST_NAMESPACE_PATTERNS.some((p) => p.test(toolInput))) {
    detections.push({
      ...
      message:
        'Host namespace sharing detected - breaks container isolation, exposes host processes/network',
    });
  }
  ...
  if (DANGEROUS_CAPABILITY_PATTERNS.some((p) => p.test(toolInput))) {
    detections.push({
      ...
      message:
        'Dangerous Linux capability detected (SYS_ADMIN/SYS_PTRACE/SYS_MODULE) - enables container escape',
    });
  }
  ...
  if (CGROUPS_MANIPULATION_PATTERNS.some((p) => p.test(toolInput))) {
    detections.push({
      ...
      message:
        'cgroups manipulation detected - classic container escape technique via release_agent',
    });
  }
  ...
  if (KERNEL_MODULE_PATTERNS.some((p) => p.test(toolInput))) {
    detections.push({
      ...
      message:
        'Kernel module loading detected - modules run in host kernel context, full system compromise',
    });
  }
  ...
  if (CHROOT_ESCAPE_PATTERNS.some((p) => p.test(toolInput))) {
    detections.push({
      ...
      message:
        'Chroot escape technique detected - attempts to break out of container filesystem isolation',
    });
  }
  ...
  if (NSENTER_PATTERNS.some((p) => p.test(toolInput))) {
    detections.push({
      ...
      message:
        'nsenter to host namespaces detected - enters host PID/network/mount namespaces, breaks isolation',
    });
  }
  ...
  if (CORE_PATTERN_EXPLOIT_PATTERNS.some((p) => p.test(toolInput))) {
    detections.push({
      ...
      message:
        'core_pattern exploitation detected - executes attacker code when process crashes, container escape',
    });
  }
  ...
  if (HOST_FILESYSTEM_MOUNT_PATTERNS.some((p) => p.test(toolInput))) {
    detections.push({
      ...
      message:
        'Host filesystem mount detected - exposes sensitive host files (/etc, /proc, /sys), data exfiltration risk',
    });
  }
  ...
  if (PROC_SYS_ESCAPE_PATTERNS.some((p) => p.test(toolInput))) {
    detections.push({
      ...
      message:
        'procfs/sysfs manipulation detected - attempts to modify host kernel parameters or access host processes',
    });
  }
  ...
  if (SECURITY_BYPASS_PATTERNS.some((p) => p.test(toolInput))) {
    detections.push({
      ...
      message:
        'Security feature bypass detected - disables AppArmor/SELinux/seccomp, removes container protections',
    });
  }
